...
const Education: React.FC<IEducationProps> = ({ 
  ...
  return (
    <Fragment>
      ...
      <SwitchWidget
        label={'Show dates'}
        value={educationInfo.showDate ?? true}
        onChange={(newValue: boolean) => {
          onChangeHandler('showDate', newValue);
        }}
      />
      {(educationInfo.showDate ?? true) ? (
        <>
          <DatePicker
            label="Start date"
        ...
        <div>Dates are hidden</div>  // Only if you want to show something when hidden
      )}
    </Fragment>
  );
};
